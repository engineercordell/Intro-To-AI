...
def buildExamplesFromExtraData(fileString="datasets/extraData.txt", limit=100000):
    examples = []
    data = open(fileString)
    lineNum = 0
    for line in data:
        ...
        for val in line.strip().split(';'):
            if count >= 30:
                if val.startswith('"') and val.endswith('"'):
                    val = val.strip('"')
                outVec[count - 30] = int(val)
            else:
                if val.startswith('"') and val.endswith('"'):
                    val = val.strip('"')
                inVec[count] = int(val) if val.isdigit() else val
            count += 1
        examples.append((inVec, outVec))
        lineNum += 1
        if lineNum >= limit:
    ...
    seperateTestAndTrain = int(len(examples) * 0.8)

    extraDataTrain = examples[:seperateTestAndTrain]
    extraDataTest = examples[seperateTestAndTrain:]

    return extraDataTrain, extraDataTest
